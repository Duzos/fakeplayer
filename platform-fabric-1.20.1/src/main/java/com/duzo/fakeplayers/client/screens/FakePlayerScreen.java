...
public class FakePlayerScreen extends Screen {
    ...
    protected void init() {
        ...
        this.input.tooltip((Text) Text.translatable(new Identifier(Fakeplayers.MOD_ID, "not_implemented_message").toTranslationKey()).getWithStyle(Style.EMPTY.withColor(TextColor.parse("red"))));
        ...
        this.confirm = ButtonWidget.builder(Text.translatable(new Identifier(Fakeplayers.MOD_ID, "screen.fakeplayerscreen.confirm").toTranslationKey()), button -> {
            player.sendMessage((Text) Text.translatable(new Identifier(Fakeplayers.MOD_ID, "not_implemented_message").toTranslationKey()).getWithStyle(Style.EMPTY.withColor(TextColor.parse("red"))), true);
            player.sendMessage((Text) Text.translatable(new Identifier(Fakeplayers.MOD_ID, "not_implemented_message").toTranslationKey()).getWithStyle(Style.EMPTY.withColor(TextColor.parse("red"))), false);
            this.close();

        })
                .dimensions((i) + (this.imageWidth/2) - (98/2),l + (128),98,20)
                // @TODO: Remove the not implemented message when the feature is implemented
                .tooltip(Tooltip.of((Text) Text.translatable(new Identifier(Fakeplayers.MOD_ID, "not_implemented_message").toTranslationKey()).getWithStyle(Style.EMPTY.withColor(TextColor.parse("red")))))
                .build();
        ...
        else if (Objects.equals(this.humanoid.getTarget(), this.player)) {
            this.stayPut.active = true;
            this.wander.active = true;
            this.follow.active = false;
        }
